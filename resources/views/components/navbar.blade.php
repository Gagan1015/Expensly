...
<nav class="bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm relative z-[60]" 
     x-data="{ 
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center lg:hidden">
                <a href="{{ route('landing') }}" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background-color: #1c50a7;">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" 
                        ...
                        <img src="{{ Auth::user()->profile_picture_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&color=1c50a7&background=EBF4FF' }}" 
                             alt="{{ Auth::user()->name }}" 
                             class="w-8 h-8 rounded-full object-cover">
                        <div class="hidden sm:block text-left">
                            <div class="text-sm font-medium text-gray-900">{{ Auth::user()->name }}</div>
                            @if(Auth::user()->isAdmin())
                                <div class="text-xs font-medium" style="color: #1c50a7;">Admin</div>
                            @endif
                        </div>
                    ...
                    <div x-show="open" 
                         ...
                         class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-[9999]">
